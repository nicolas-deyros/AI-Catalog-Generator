...
const fileToResizedBase64 = (file: File, maxSize: number = 512): Promise<string> => {
    return new Promise((resolve, reject) => {
        if (!file.type.startsWith('image/')) {
            return reject(new Error(`File is not a supported image type: ${file.name}`));
        }
        ...
        reader.onload = (event) => {
            ...
            img.onerror = () => reject(new Error(`Failed to load image file: ${file.name}. It may be corrupted or an unsupported format.`));
        };
...
const ImageUploader: React.FC<ImageUploaderProps> = ({ items, onItemsChange }) => {
  ...
  const handleFileChange = async (files: FileList | null) => {
    ...
    if (failedFiles.length > 0) {
        alert(`Could not process ${failedFiles.length} file(s). They may be corrupted or in an unsupported format. Please check the console for details.`);
    }
  };
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <p className="mt-4 text-lg text-gray-600">Drag & drop your product images here</p>
      ...
      {items.length > 0 && (
        <div className="mt-6">
            <h3 className="text-lg font-medium text-gray-800">Uploaded Products ({items.length})</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mt-4">
                {items.map((item) => (
                    <div key={item.id} className="relative group bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col">
                        ...
                        <button onClick={() => removeItem(item.id)} className="absolute -top-2 -right-2 p-1 bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100">
